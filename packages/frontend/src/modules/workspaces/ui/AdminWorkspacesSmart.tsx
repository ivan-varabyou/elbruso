...
import { Plus, Search, Filter, Loader2, FileText, Trash2, Edit } from "lucide-react";
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-zinc-500 text-sm">Управление системными шаблонами для новых пользователей</p>
        </div>
        <button
      ...
      <div className="flex flex-wrap gap-4 items-center bg-white p-4 rounded-xl border border-zinc-200">
        ...
        >
          ...
          {organizations.map(o => <option key={o.id} value={o.id}>{o.name_ru}</option>)}
        </select>
        ...
        >
          ...
          {sports.map(s => <option key={s.id} value={s.id}>{s.name_ru}</option>)}
        </select>
      ...
      <div className="bg-white rounded-xl border border-zinc-200 overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-zinc-500 text-sm">Попробуйте изменить параметры поиска или создайте новый шаблон</p>
          </div>
        ) : (
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-zinc-50 border-b border-zinc-200">
                <th className="px-6 py-3 text-[11px] font-bold text-zinc-400 uppercase tracking-wider">Название</th>
                <th className="px-6 py-3 text-[11px] font-bold text-zinc-400 uppercase tracking-wider">Организация</th>
                <th className="px-6 py-3 text-[11px] font-bold text-zinc-400 uppercase tracking-wider">Вид спорта</th>
                <th className="px-6 py-3 text-[11px] font-bold text-zinc-400 uppercase tracking-wider text-right">Действия</th>
              </tr>
            </thead>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {template.description && (
                          <div className="text-xs text-zinc-500 line-clamp-1">{template.description}</div>
                        )}
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4 text-sm text-zinc-600">
                    {organizations.find(o => Number(o.id) === template.organization_id)?.name_ru || "—"}
                  </td>
                  <td className="px-6 py-4 text-sm text-zinc-600">
