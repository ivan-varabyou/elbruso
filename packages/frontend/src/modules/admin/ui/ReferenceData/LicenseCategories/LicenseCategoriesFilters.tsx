...
interface LicenseCategoriesFiltersProps {
  sportId?: number;
  onFilterChange: (filters: { sportId?: number; search?: string }) => void;
}

export function LicenseCategoriesFilters({ sportId, onFilterChange }: LicenseCategoriesFiltersProps) {
  ...
  useEffect(() => {
    const fetchSports = async () => {
      try {
        ...
        setSports((response as any).data || []);
      } catch (error) {
