...
import { ColDef, CellValueChangedEvent, CellClickedEvent, GridReadyEvent, ModuleRegistry, AllCommunityModule } from 'ag-grid-community';
...
export function DynamicTable({ tableId, workspaceId }: DynamicTableProps) {
  ...
  const columnDefs = useMemo<ColDef[]>(() => {
    ...
    const rowNumberCol: ColDef = {
      ...
      editable: false,
      cellStyle: { backgroundColor: '#fafafa', fontWeight: '500', color: '#71717a', textAlign: 'center' },
    };
  ...
  const onCellMouseOver = (event: any) => {
    ...
    document.querySelectorAll('.ag-cell.active-cell, .ag-cell.active-header, .ag-header-cell.active-header').forEach(el => {
      el.classList.remove('active-cell', 'active-header');
    });
    
    // Highlight only the active cell
    const activeCell = document.querySelector(`.ag-row[row-index="${rowIndex}"] .ag-cell[col-id="${colId}"]`);
    if (activeCell) {
      activeCell.classList.add('active-cell');
    }
    
    // Highlight row number (parent)
    const rowNumberCell = document.querySelector(`.ag-row[row-index="${rowIndex}"] .ag-cell[col-id="rowNumber"]`);
    if (rowNumberCell) {
