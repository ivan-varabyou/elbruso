  ...
  if (loading)
    return (
      <div className="p-8 text-center animate-pulse text-zinc-400">
        Загрузка структуры организаций...
      </div>
    );
        ...
        <div className="bg-orange-50 border border-orange-100 rounded-2xl p-6 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-3">
              <label className="text-xs font-bold text-orange-700 uppercase tracking-widest">
                Переключить роль
              </label>
        ...
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          <span
            className={cn(
              ...
              !isRoot && "text-zinc-700 group-hover:text-blue-600",
            )}
          >
            {node.name_ru}
          </span>
          {node.internal_code && (
            <code
              className={cn(
                "text-[9px] px-1.5 py-0.5 rounded uppercase font-mono tracking-tighter shrink-0",
                isRoot
                  ? "bg-white/20 text-white"
                  : "bg-zinc-100 text-zinc-400 group-hover:bg-blue-50 group-hover:text-blue-400",
              )}
            >
              {node.internal_code}
            </code>
