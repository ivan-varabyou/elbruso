...
import { BoxPlotConfig, BoxPlotDataPoint } from "../../../types/visualization";
...
export class BoxPlotAdapter extends BaseChartAdapter<BoxPlotConfig> {
  protected render(): void {
    ...
    const stats: BoxPlotStats[] = data.map((d: BoxPlotDataPoint) => {
      const sorted = d.values.slice().sort((a: number, b: number) => a - b);
    ...
    const categoryScale = d3
      .scaleBand<string>()
      .domain(data.map((d: BoxPlotDataPoint) => d.category))
      .range([0, isVertical ? width : height])
      .padding(0.3);

    const allValues = data.flatMap((d: BoxPlotDataPoint) => d.values);
    const valueExtent = d3.extent(allValues) as [number, number];
    const valueScale = d3
      .scaleLinear()
      .domain([(valueExtent[0] ?? 0) * 0.95, (valueExtent[1] ?? 0) * 1.05])
    ...
    stats.forEach((stat: BoxPlotStats, i: number) => {
      ...
      if (isVertical) {
        ...
        [stat.min, stat.max].forEach((value: number) => {
          boxGroup
            .append("line")
      ...
      if (showOutliers) {
        stat.outliers.forEach((outlier: number) => {
          boxGroup
            .append("circle")
