...
import { BarChartConfig, BarChartDataPoint } from "@/shared/visualization";
...
export class BarChartAdapter extends BaseChartAdapter<BarChartConfig> {
  ...
  private renderVerticalBars(width: number, height: number, data: BarChartDataPoint[]) {
    const self = this;
    // Create scales
    const xScale = d3
      .scaleBand()
    ...
    const yScale = d3
      .scaleLinear()
      .domain([0, d3.max(data, (d: BarChartDataPoint) => d.value) || 100])
      .nice()
      .range([height, 0]);
    ...
    this.g!.selectAll(".bar")
      ...
      .on("mouseover", (event: globalThis.MouseEvent, d: BarChartDataPoint) => {
        if (self.tooltip) {
          const content = self.config.tooltip?.formatter
            ? self.config.tooltip.formatter(d)
            : `${d.label}<br/>${formatNumber(d.value)}`;
          showTooltip(self.tooltip, content, event);
        }
        d3.select(event.currentTarget as Element).attr("opacity", 0.8);
      })
      .on("mouseout", (event: globalThis.MouseEvent) => {
        if (self.tooltip) {
  ...
  private renderHorizontalBars(width: number, height: number, data: BarChartDataPoint[]) {
    const self = this;
    // Create scales
    const yScale = d3
      .scaleBand()
