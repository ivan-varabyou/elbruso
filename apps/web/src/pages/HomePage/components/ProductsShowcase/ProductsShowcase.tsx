  ...
  const defaultUSPs = [
    {
      ...
      features: ["Validation", "Cleaning", "Reporting"],
    },
    {
      id: "engine",
      label: "Engine",
      title: "Analytics Engine",
      description: "Powerful analytics processing",
      features: ["Fast Processing", "Real-time", "Scalable"],
    },
  ];

  const USPs =
    productsShowcase?.usps?.map((usp: any) => ({
      ...usp,
      icon: ICON_MAP[usp.id] || Search,
    })) || defaultUSPs;

  const [activeTab, setActiveTab] = useState(USPs[0]?.id);
  const currentUSP = USPs.find((u: any) => u.id === activeTab) || USPs[0];
    ...
    <section className="pt-12 bg-white overflow-hidden">
      <div className="container">
        <div className="text-center mb-8">
          ...
          >
            {productsShowcase?.title || "Products"}
          </motion.h2>
          <p className="text-elbruso-text-muted max-w-2xl mx-auto text-lg">
            {productsShowcase?.subtitle || "Subtitle here"}
          </p>
        </div>
            ...
            <div className="lg:col-span-5 space-y-10">
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 bg-primary-blue rounded-12 flex items-center justify-center text-white shadow-lg shadow-primary-blue/20">
                    {currentUSP?.icon && <currentUSP.icon size={24} />}
                  </div>
              ...
              <div className="space-y-5">
                <p className="font-bold text-xs uppercase tracking-widest text-[#1A73E8]">
                  {productsShowcase?.coreCapabilities || "Features"}
                </p>
