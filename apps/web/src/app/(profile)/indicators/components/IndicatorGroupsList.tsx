...
export function IndicatorGroupsList() {
    ...
    const fetchGroups = async () => {
        setLoading(true);
        try {
            const data = await referencesApi.getIndicatorGroups();
            setGroups(data);
        } catch (error) {
    ...
    const handleDelete = async (id: number) => {
        if (!confirm('Вы уверены, что хотите удалить эту группу? Это не удалит индикаторы, но они перестанут быть к ней привязаны.')) return;
        try {
            await referencesApi.deleteIndicatorGroup(id);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {loading ? (
                    Array.from({ length: 6 }).map((_, i) => (
                        <div key={i} className="h-32 bg-zinc-100 animate-pulse rounded-2xl border border-zinc-200" />
                    ))
                ) : filteredGroups.length > 0 ? (
                    filteredGroups.map((group) => (
                        <div key={group.id} className="group bg-white p-5 rounded-2xl border border-zinc-200 hover:border-blue-300 hover:shadow-md transition-all duration-300 relative overflow-hidden">
                            <div className="flex items-start justify-between relative z-10">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h3 className="font-bold text-zinc-900 group-hover:text-blue-700 transition-colors">{group.name_ru}</h3>
                                        <code className="text-[10px] font-mono text-zinc-400 uppercase tracking-tight">{group.code}</code>
                                    </div>
                                </div>
                    ...
                    <div className="col-span-full py-20 text-center bg-zinc-50/50 rounded-3xl border border-dashed border-zinc-200">
                        ...
                        <p className="text-sm text-zinc-500 mt-1">Создайте новую группу, чтобы начать классификацию</p>
                    </div>
                )}
            </div>
