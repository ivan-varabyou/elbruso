...
import { GenerateIndicatorsModal } from './components/GenerateIndicatorsModal';
...
export default function IndicatorsPage() {
    ...
    const fetchIndicators = async () => {
        setLoading(true);
        try {
            const data = await referencesApi.getIndicators({
                search: searchQuery,
    ...
    const handleDelete = async (id: string | number) => {
        ...
        try {
            await referencesApi.deleteIndicator(String(id));
                            ...
                            <table className="w-full text-left text-sm border-collapse">
                                <thead>
                                    <tr className="bg-zinc-50/30 border-b border-zinc-100">
                                        <th className="px-6 py-4 font-semibold text-zinc-500 uppercase tracking-wider text-[10px]">Индикатор</th>
                                        <th className="px-6 py-4 font-semibold text-zinc-500 uppercase tracking-wider text-[10px]">Статус / Область</th>
                                        <th className="px-6 py-4 font-semibold text-zinc-500 uppercase tracking-wider text-[10px] text-center">Вес</th>
                                        <th className="px-6 py-4 font-semibold text-zinc-500 uppercase tracking-wider text-[10px] text-right">Управление</th>
                                    </tr>
                                </thead>
                                    ...
                                    {loading ? (
                                        Array.from({ length: 6 }).map((_, i) => (
                                            <tr key={i} className="animate-pulse">
                                                <td className="px-6 py-5"><div className="h-5 bg-zinc-100 rounded-lg w-2/3"></div><div className="h-3 bg-zinc-50 rounded mt-2 w-1/4"></div></td>
                                                <td className="px-6 py-5"><div className="h-6 bg-zinc-100 rounded-full w-24"></div></td>
                                                <td className="px-6 py-5"><div className="h-5 bg-zinc-100 rounded-lg w-8 mx-auto"></div></td>
                                                <td className="px-6 py-5"><div className="h-8 bg-zinc-100 rounded-lg w-20 ml-auto"></div></td>
                                            </tr>
                                        ))
                                    ) : indicators.length > 0 ? (
                                        indicators.map((indicator) => {
                                            ...
                                            return (
                                                <tr key={indicator.id} className="hover:bg-blue-50/30 transition-colors group/row">
                                        ...
                                        <tr>
                                            <td colSpan={4} className="px-6 py-20 text-center">
                                                <div className="flex flex-col items-center gap-3 text-zinc-400">
                                                    ...
                                                    <p className="text-xs max-w-[200px]">Попробуйте изменить фильтры или поисковый запрос</p>
                                                    <Button variant="outline" size="sm" className="mt-4" onClick={() => { setSelectedScopes([]); setSearchQuery(''); }}>
                                                        Сбросить фильтры
                                                    </Button>
                                                </div>
                                            </td>
...
import { IndicatorGroupsList } from './components/IndicatorGroupsList';
