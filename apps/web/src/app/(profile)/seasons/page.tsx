...
export default function SeasonsPage() {
    ...
    const handleDelete = async (id: string | number) => {
        if (confirm('Вы уверены, что хотите удалить этот сезон? Это может повлиять на привязанные рабочие области.')) {
            try {
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-zinc-50 border-b border-zinc-200">
                                <th className="px-5 py-3 text-[11px] font-bold text-zinc-500 uppercase tracking-wider">Название</th>
                                <th className="px-5 py-3 text-[11px] font-bold text-zinc-500 uppercase tracking-wider">Период</th>
                                <th className="px-5 py-3 text-[11px] font-bold text-zinc-500 uppercase tracking-wider">Вид спорта</th>
                                <th className="px-5 py-3 text-[11px] font-bold text-zinc-500 uppercase tracking-wider text-right">Действия</th>
                            </tr>
                        </thead>
                                            ...
                                            <div className="flex flex-wrap gap-1.5">
                                                {season.sports && season.sports.length > 0 ? (
                                                    season.sports.map(sport => (
                                                        <div key={sport.id} className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-blue-50 text-blue-700 text-[10px] font-bold border border-blue-100 uppercase">
