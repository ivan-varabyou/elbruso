...
const histogramData = [
  ...
  ...Array(25)
    .fill(0)
    .map(() => Math.random() * 15 + 25),
  ...Array(10)
    .fill(0)
    .map(() => Math.random() * 10 + 35),
];
...
export const LiveSportsChart = ({
  ...
  const renderMainBarChart = (
    ...
    g.selectAll("rect.bar")
      ...
      .attr("rx", 6)
      .attr("fill", (_d, i) => gradients[i % gradients.length])
      .attr("filter", "url(#chart-glow)")
      .attr("width", 0)
      .transition()
      .duration(1500)
    ...
    g.selectAll("text.value")
      ...
      .transition()
      .duration(1500)
      .tween("text", function (this: SVGTextElement, d: BarDataPoint) {
  ...
  const renderGroupedBarChart = (
    ...
    const z = d3
      .scaleOrdinal()
      .domain(categories)
      .range([colorPalette.vibrant[0], colorPalette.vibrant[1], colorPalette.vibrant[2]]);
    ...
    g.append("g")
      ...
      .selectAll("rect")
      .data((d) => {
        const categoryValues = d as GroupedDataPoint & { [key: string]: number };
        return categories.map((key) => ({ key, value: categoryValues[key] }));
      })
      .enter()
      .append("rect")
      .attr("x", (d) => x1(d.key)!)
  ...
  const renderStackedBarChart = (
    ...
    const keys = ["beginner", "intermediate", "advanced"] as const;
    const series = d3.stack<StackedDataPoint>().keys(keys)(data);
    ...
    const z = d3
      .scaleOrdinal()
      .domain(keys)
      .range([colorPalette.vibrant[3], colorPalette.vibrant[4], colorPalette.vibrant[5]]);
    ...
    type SeriesDatum = d3.SeriesPoint<StackedDataPoint>;
  ...
  const renderPieChart = (
    ...
    arcs
      ...
      .transition()
      .duration(1500)
      .attrTween("d", function (d: d3.PieArcDatum<PieDataPoint>) {
  ...
  const renderAreaChart = (
    ...
    g.append("path")
      .datum(data)
      .attr("fill", "none")
      .attr("stroke", colorPalette.gradient4[0])
      .attr("stroke-width", 3)
      .attr("d", line);
  };
  ...
  const renderStreamGraph = (
    ...
    const keys = ["football", "basketball", "hockey"] as const;
    const series = d3.stack<StreamDataPoint>().keys(keys).offset(d3.stackOffsetWiggle)(data);
    ...
    const color = d3
      .scaleOrdinal()
      .domain(keys)
      .range([colorPalette.vibrant[6], colorPalette.vibrant[7], colorPalette.vibrant[0]]);

    type StreamSeriesDatum = d3.SeriesPoint<StreamDataPoint>;

    const area = d3
      .area<StreamSeriesDatum>()
      .x((d) => x(d.data.year))
      .y0((d) => y(d[0]))
      .y1((d) => y(d[1]))
      .curve(d3.curveBasis);
  ...
  const renderWaterfallChart = (
    ...
    g.selectAll("rect")
      ...
      .attr("fill", (d) =>
        d.type === "positive"
          ? colorPalette.vibrant[3]
          : d.type === "negative"
            ? colorPalette.vibrant[0]
  ...
  const renderHistogram = (
    ...
    g.selectAll("rect")
      ...
      .attr("height", 0)
      .attr("fill", (_d, i: number) => colorScale(i))
      .attr("rx", 3)
      .transition()
      .duration(1500)
      .attr("y", (d) => y(d.length))
  ...
  const renderSunburst = (
    ...
    const root = d3.hierarchy<SunburstNode>(data).sum((d) => d.value || 0);
    partition(root);
    ...
    g.selectAll("path")
      ...
      .attr("fill", (_d, i: number) => color(i.toString()))
      .attr("stroke", "white")
      .attr("stroke-width", 2)
      .attr("opacity", 0)
      .transition()
  ...
  const renderRadialBar = (
    ...
    g.selectAll("path")
      ...
      .attr("d", (d) => arc({ ...d, value: 0 })!)
      .transition()
      .duration(1500)
      .attrTween("d", function (this: SVGPathElement, d: RadialBarDataPoint) {
        const i = d3.interpolate(0, d.value);
        return (t: number) => arc({ ...d, value: i(t) })!;
      });
  };
