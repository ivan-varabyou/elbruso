...
export class IndicatorsService implements OnModuleInit {
  ...
  async findAll(filters: IndicatorFiltersDto): Promise<IndicatorCatalog[]> {
    ...
    if (filters.userRole !== 'ADMIN') {
      ...
      query = query.where((eb: any) => {
        ...
        const sportCond = userSportId ? eb.and([eb('organization_id', 'is', null), eb('sport_id', '=', userSportId)]) : null;
        
        // Federation scope now includes ancestors
        const fedOrgs = Array.from(new Set([...(ancestorOrgIds || []), ...(userOrganizationId ? [userOrganizationId] : [])]));
        ...
        if (activeScopes.length === 0) {
          ...
          if (activeScopes.includes('federation') && federationCond) conditions.push(federationCond);
    ...
    if (filters.search) {
      query = query.where('indicator_catalog.name_ru' as any, 'ilike' as any, `%${filters.search}%`);
    }
  ...
  async generate(dto: GenerateIndicatorsDto): Promise<any[]> {
    ...
    // 1. Fetch templates
    let templatesQuery = this.db.client.selectFrom('indicator_generation_templates' as any).selectAll();

    if (templateIds && templateIds.length > 0) {
      templatesQuery = templatesQuery.where('id' as any, 'in', templateIds as any);
    } else {
      if (sportId) {
        templatesQuery = templatesQuery.where('sport_id' as any, '=', sportId as any);
      }
      if (category) {
        ...
        if (!isNaN(Number(category))) {
          templatesQuery = templatesQuery.where('category_id' as any, '=', Number(category) as any);
        }
      }
    }
    ...
    for (const template of templates) {
      try {
        const result = await sql`SELECT * FROM generate_indicators_from_template_v5(${template.id}, ${overwrite || false})`.execute(this.db.client);
        results.push(...(result.rows as any[]));
      } catch (err: any) {
        this.logger.error(`Failed to generate from template ${template.id}: ${err.message}`);
      }
    }
